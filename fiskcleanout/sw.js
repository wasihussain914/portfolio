/* FiskCleanout service worker.
 *
 * Served from <basePath>/sw.js (static export; basePath is '' on the
 * root-domain deploy fiskcleanout.com and '/fiskcleanout' on the
...
// domain root, '/fiskcleanout' on the legacy subpath deploy.
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    data = { title: 'FiskCleanout', body: event.data ? event.data.text() : '' };
  }
  const title = data.title || 'FiskCleanout';
  event.waitUntil(
